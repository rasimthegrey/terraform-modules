locals {
  ...
  env_vars_keys        = var.map_environment != null ? keys(var.map_environment) : var.environment != null ? [for m in var.environment : m["name"]] : []
  env_vars_values      = var.map_environment != null ? values(var.map_environment) : var.environment != null ? [for m in var.environment : m["value"]] : []
  ...
  sorted_environment_vars = [
    ...
    {
      name  = key
      value = local.env_vars_as_map[key]
    }
  ]

  # Sort secrets so terraform will not try to recreate on each plan/apply
  secrets_keys        = var.map_secrets != null ? keys(var.map_secrets) : var.secrets != null ? [for m in var.secrets : m["name"]] : []
  secrets_values      = var.map_secrets != null ? values(var.map_secrets) : var.secrets != null ? [for m in var.secrets : m["valueFrom"]] : []
  ...
  log_configuration_with_null = var.log_configuration == null ? null : {
    logDriver = tostring(var.log_configuration["logDriver"])
    options   = tomap(var.log_configuration["options"])
    secretOptions = local.log_configuration_secret_options == null ? null : [
      for secret_option in tolist(local.log_configuration_secret_options) : {
        name      = tostring(secret_option["name"])
        valueFrom = tostring(secret_option["valueFrom"])
      }
    ]
  }
